...
public class OrderController {
    ...
    public ResponseEntity<?> placeOrder(@RequestParam int cartId, @RequestParam String address) {
        try {
    ...
    public ResponseEntity<?> updateOrderStatus(@RequestParam int orderId) {
        ...
        else return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Not allowed to access this resource");
    }

    // End point to cancel the order
    @PreAuthorize("hasRole('USER')")
    @DeleteMapping("/deleteorder")
    public ResponseEntity<String> cancelOrderByUserId(@RequestParam int orderId) {
        try {
